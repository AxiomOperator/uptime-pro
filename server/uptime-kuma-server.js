...
class UptimeKumaServer {
    ...
    async getMonitorJSONList(userID, monitorID = null) {
        ...
        if (monitorID) {
            whereClause.id = monitorID;
        }
        const rows = await prisma.monitor.findMany({
            where: whereClause,
            orderBy: [ { weight: "desc" }, { name: "asc" } ],
        });
        let monitorList = rows.map(r => Object.assign(new Monitor(), r));
    ...
    async loadMaintenanceList(userID) {
        const maintenanceRows = await prisma.maintenance.findMany({ orderBy: [ { end_date: "desc" }, { title: "asc" } ] });
        let maintenanceList = maintenanceRows.map(r => Object.assign(new Maintenance(), r));
